...
import d.spidchenko.sampledaydream.util.SoundEngine

private const val TAG = "DreamSurfView.LOG_TAG"
private const val CHARGED_ACTION_KEY = "action_when_charged"


@SuppressLint("ViewConstructor")
class DreamSurfaceView(context: Context, preferences: SharedPreferences, soundEngine: SoundEngine? = null) : GLSurfaceView(context) {
    ...
    val batteryInfoReceiver: BroadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            if ((preferences.getString(CHARGED_ACTION_KEY, "none")).equals("play_sound")) {
                    ...
                    Log.d(TAG, "onReceive: isCharged? $isCharged")
                    if (isCharged) {
                        Log.d(TAG, "onReceive: CHARGED!")
                        soundEngine?.playPop()
                }
            }
        }
